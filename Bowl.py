...
def bowling(balls):
    ...
    ## bowling([int, ...]) -> int
    ## your code here
    #make numerical boxscore
    return sum(numerical_boxscore(balls))

def numerical_boxscore(balls):
    boxscore = [0]*21
    index = 0
    for n,b in enumerate(balls):
        #special case, last three frames
        if index >= 18: 
            boxscore[index] = b
        #bottom of frame
        elif index % 2 == 1: 
            #spare
            if b + boxscore[index-1] == 10: 
                boxscore[index] = b + balls[n+1]
            #completed frame, pins stnding
            else: 
                boxscore[index] = b 
        #strike
        elif b == 10: 
            boxscore[index] = 10 + balls[n+1] + balls[n+2]
            index += 1
            boxscore[index] = 0
        #bowled top frame, pins standing
        else:
            boxscore[index] = b
        index += 1
    return boxscore
    
def make_boxscore(balls):
    boxscore = [0]*21
    index = 0
    for b in balls:
        #special case, last three frames
...
print test_bowling()
#print make_boxscore([0,1]*6+[10]+[0,10]+[1,9]+[10,9,1])
#print numerical_boxscore([0,1]*6+[10]+[0,10]+[1,9]+[10,9,1])
