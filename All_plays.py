...
def all_plays(hand, board):
    ...
    ###Your code here.
    return set([(pos, ACROSS, word ) for pos,word in hplays] + 
               [((i,j), DOWN, word ) for (j,i),word in vplays])
                    #this comprehension works either way: 
                    #refering to pos on its own or drilling down to the tuple elems
                    #because python iterables are boss
