...
def best_wild_hand(hand):
    #jokers
    #'?B' and '?R'
    #use itertools.product and *map with a separate replace function instead
    handnoJ =  list(itertools.ifilterfalse(lambda x: x[0] == '?', hand))
    print handnoJ
    return max([h for h_list in [
        itertools.combinations(h_jokeropt, 5) 
        for h_jokeropt in 
        [[s + b] + [s + r] + handnoJ 
        for s in ['2','3','4','5','6','7','8','9','T','J','Q','K','A']  
        for b in ['C','S'] if '?B' in hand  
        for r in ['D','H'] if '?R' in hand]] 
        for h in h_list], key=hand_rank)  
        #errors out if there is no ?R or ?B
        #probably not a great place to use only list comprehensions
