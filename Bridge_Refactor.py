...
def bsuccessors3(state):
    ...
    arrows = ['->','<-']
    return dict(((here  - frozenset([a,b][0:(1-light)*2]) | frozenset([a,b][0:(light)*2]),
                  there - frozenset([a,b][0:(light)*2]) | frozenset([a,b][0:(1-light)*2]),
                  1-light ), # instead of 1- light you could say 'not light' since T:F and 0:1 are treated the same
                 (set([a, b]), arrows[light]))
