...
def anagrams(phrase, shortest=2):
    ...
    # your code here
    phrase = ''.join(phrase.split()) #remove all spaces
    words = find_words(phrase)
    results = set()
    
    def find_ana(phr, wds, shortest, ana):
        for word in wds:
            if len(word) >= shortest:
                ...
                if w == set():
                    if not p:
                        results.add(' '.join(sorted(a)))
                        #print 'result found + ', a
                    a = ana
                    #print 'emptying ana '
                else:
                    find_ana(p, w, shortest, a)
        return
    find_ana(phrase, words, shortest, [])
    return results
