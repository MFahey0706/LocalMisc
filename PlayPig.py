...
def play_pig(A, B):
    ...
    # your code here
    state = (0,0,0,0)
    while state[1] < 50 and state[2] < 50:
        if state[0]:
            state = globals()[A(state)](state,random.randint(1,6))  #global generates a dict or the current symbol table
        else:
            state = globals()[B(state)](state,random.randint(1,6))
    return A if state[1] >= 50 else B
