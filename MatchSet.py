...
def matchset(pattern, text):
    ...
    if 'lit' == op:
        ...
        return set(chain.from_iterable([matchset(('lit',c), text) for c in x])) # use itertools.chain.from_iterable for multi-union
        # set([text[1:]]) if text.startswith(x) else null #doable bc str.startswith() accepts tuples as arguments
        # could also use 'if any(text.startswith(c) for c in x)' because apparently any accepts comprehensions
    elif 'eol' == op:
...
def test2():
    assert matchset(('oneof', 'ab'), 'bc123')           == set(['c123'])
    return 'pass'
