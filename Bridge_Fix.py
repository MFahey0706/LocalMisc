...
def bridge_problem(here):
    ...
    while frontier:
        path = frontier.pop(0)
        if not path[-1][0] or path[-1][0] == frozenset(['light']): ## That is, nobody left here
            return path
        else:
...
print bridge_problem([1.3,18])
[(frozenset(['light', 18, 1.3]), frozenset([]), 0), (1.3, 18, '->'), (frozenset(
[]), frozenset(['light', 18, 1.3]), 18)]
