...
def strategy_d(state):
    (p, me, you, pending, double) = state 
    dbl_agnostic = tuple(list(state)[0:4]+[0])
    if double == 'double':
        return 'accept' if Pwin(dbl_agnostic) > .9 else 'decline'
    if (double == 1 and Pwin(dbl_agnostic) > .9):
        return 'double'  
    else:
        return best_action(dbl_agnostic, pig_actions_d, Q_pig, Pwin)
...
def Q_pig(state, action, Pwin):  
    ...
    if action == 'roll':
        return (1 - Pwin(do(action, state, loaded_die(1)))
                + sum(Pwin(do(action, state, loaded_die(d))) for d in (2,3,4,5,6))) / 6.
    print action, state
    raise ValueError
...
def Pwin(state):
    ...
    if me + pending >= goal:
        ...
        return max(Q_pig(state, action, Pwin)
                   for action in pig_actions_d(tuple(list(state)[0:4]+[0])))
                   #Pwin should always be called as if double is unavailable
