...
def is_letter(sq):
    return isinstance(sq, str) and sq in (LETTERS + map(str.lower,LETTERS))
    #didn't need to change this for this exercise, since we are only finding single plays
    #but seemed good to anticipate blank tiles existing on the board already
...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    i = start + len(pre)
    p_up = str.upper(pre)
    if p_up in WORDS and anchored and not (is_letter(row[i])): 
    ...
    if p_up in PREFIXES:       
        ...
        if is_letter(sq):
            ...
            #checking if the sq is an anchor; if so it already contains a set of possible letters
            for L in hand:
                if L == '_':
                #WILDBLOCK#
                    for wildcard in possibilities:
                        add_suffixes(hand.replace(L, '', 1), pre+str.lower(wildcard), start, row, results)
                #/WILDBLOCK#
...
def find_prefixes(hand, pre='', results=None):
    ...
    p_up = str.upper(pre)
    if p_up in WORDS or p_up in PREFIXES: results.add(pre)
    if p_up in PREFIXES:
        for L in hand:
        #WILDBLOCK#
            if L == '_':
                for wildcard in LETTERS:
                    find_prefixes(hand.replace(L, '', 1), pre+str.lower(wildcard), results)
            else:
        #/WILDBLOCK#
...
def calculate_score(board, pos, direction, hand, word):
    ...
    for (n, L) in enumerate(word):
        ...
        if str.isupper(L): #only do letter bonus and add to total if nonblank tile
        #could've also just added all lowercase letters to the POINTS dict with a value of 0
            letter_mult = (1 if is_letter(sq) else
                           3 if b == TL else 2 if b == DL else 1)
            total += POINTS[L] * letter_mult 
        if isinstance(sq, set) and sq is not ANY and direction is not DOWN: #sq is an anchor in other dir
