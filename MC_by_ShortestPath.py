...
def mc_problem2(start=(3, 3, 1, 0, 0, 0), goal=None):
    ...
    if goal == None:
        def mc_goal(state): return state == (0, 0, 0) + start[:3]
    else:
        def mc_goal(state): return state == goal

    return shortest_path_search(start, csuccessors, mc_is_goal) # <== insert arguments here
