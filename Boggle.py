...
def boggle_words(board, minlength=3):
    ...
    def build_words(avail_board, loc, pref): #could've tracked path rather than board
        nbors = neighbors(loc,N)
        for nbr in nbors:
            sq = avail_board[nbr]
            if is_letter(sq):
                new_pref=pref+sq
                if new_pref in WORDS and len(new_pref) >= minlength:
                    results.add(new_pref)
                if new_pref in PREFIXES:
                    build_words(avail_board[:loc]+'.'+avail_board[loc+1:], 
                                nbr, new_pref)
    #init
    for i,start in enumerate(board):
        if is_letter(start):
            build_words(board, i, start)
    #ret
    return results
...
print test()
#print boggle_words(Board('XXXX TEST XXXX XXXX'))
