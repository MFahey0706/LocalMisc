...
WORDS, PREFIXES = readwordlist('testlist.txt')
...
def find_prefixes(hand, pre='', results=None):
    ...
    try:
        print 'Hand: ' + hand
        return memo_dict[hand]
    except KeyError:
        def recurse_prefixes(hand, pre, results):
            if results is None: results = set()
            if pre in PREFIXES:
                results.add(pre)
                for L in hand:
                    recurse_prefixes(hand.replace(L, '', 1), pre+L, results)
            return results
        results = recurse_prefixes(hand, pre, results)
        print 'Result archived'
        memo_dict[hand]=results
        return results
        
print PREFIXES
print find_prefixes('CA')
print find_prefixes('BA')
print find_prefixes('AC')