...
def collapse_to_max(tree):
    for i in reversed(range(2,len(tree),2)): #iterate on even (left) nodes; tree len is even; range is through last (odd) index
        #print "Children:",i, tree[i], tree[i+1]
        tree[i//2]+=max(tree[i],tree[i+1]) #add the max of children to parent, ending w/ an update of node 1
        #print "Parent:",tree[i//2]
    return tree[1]
